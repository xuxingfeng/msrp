    ...
    implements Observer
{
    ...
    {
//        System.out.println("r200ProcessRequest,message-id:"+transaction.getMessageID()+",tid:"+transaction.getTID());
        ...
        if (transaction.getTransactionType() == TransactionType.SEND)
        {
            ...
            System.out.println("receivedBytes:"+message.getReceivedBytes());
            if (message != null && message.isComplete())
            {
                ...
                Session session = getAssociatedSession(transaction);//fxx modified
...
//            else if(null != message && message.wasAborted()){//fxx add start
//                logger.trace(String.format(
//                        "%s %s has an associated message-id[%s] that is aborted.",
//                        this, transaction, transaction.getMessageID()));
...
//            }//fxx add end
    ...
    {
        logger.info("进入generateTransactionsToSend");
        ...
        //fxx add end
        while(chunks>0 && validated.getSession().getConnection() != null) {
            logger.info("holdsLock:" + Thread.holdsLock(this));
            if(preTransaction!=null && !preTransaction.hasResponse()){
                try {
                    Thread.sleep(100);
                ...
                //fxx modified start
                validated.setChunksSent(validated.getChunks()-chunks);
                byte flag = Transaction.FLAG_IRQ;//+
                if (chunks == 1) {
                    flag = Transaction.FLAG_END;//$
                }
//                if (validated.aborted) {
//                    flag = Transaction.FLAG_ABORT;//#
//                }
                Transaction newTransaction = new Transaction(validated, this, flag);
                preTransaction = newTransaction;
//            Transaction newTransaction = new Transaction((OutgoingMessage) validated);
                //fxx modified end
                synchronized (this) {
...
//                }
        ...
        if(chunks==0){
            validated.setChunksSent(validated.getChunks());
        }
        /*do
        {
            //fxx modified start
